...
public class SmartMovingRender extends SmartRenderContext {
	...
	public SmartMovingRender(IRenderPlayer irp) {
		...
		SmartMovingModel modelArmor = irp.getMovingModelArmor()
				.getMovingModel();
	...
	public void doRender(AbstractClientPlayer entityplayer, double d, double d1,
		...
		if (moving != null) {
			boolean isInventory = d == 0.0F && d1 == 0.0F && d2 == 0.0F
					&& f == 0.0F && renderPartialTicks == 1.0F;

			boolean isClimb = moving.isClimbing && !moving.isCrawling
					&& !moving.isCrawlClimbing && !moving.isClimbJumping;
			...
			boolean isCrawlClimb = moving.isCrawlClimbing
					|| (moving.isClimbing && moving.isCrawling);
			...
			SmartStatistics statistics = SmartStatisticsFactory
					.getInstance(entityplayer);
			float currentHorizontalSpeedFlattened = statistics != null
					? statistics.getCurrentHorizontalSpeedFlattened(
			...
			if (!isInventory && entityplayer.isSneaking()
					&& !(entityplayer instanceof EntityPlayerSP) && isCrawl)
				d1 += 0.125D;
		}
		...
		if (moving != null && moving.isLevitating && modelPlayers != null)
			for (int i = 0; i < modelPlayers.length; i++)
				modelPlayers[i]
						.getMovingModel().md.currentHorizontalAngle = modelPlayers[i]
	...
	public void rotateCorpse(AbstractClientPlayer entityplayer, float totalTime,
		...
		SmartMoving moving = SmartMovingFactory.getInstance(entityplayer);
		if (moving != null) {
			boolean isInventory = f2 == 1.0F && moving.isp != null && moving.isp
					.getMcField().currentScreen instanceof GuiInventory;
			if (!isInventory) {
				float forwardRotation = entityplayer.prevRotationYaw
						+ (entityplayer.rotationYaw
								- entityplayer.prevRotationYaw) * f2;

				if (moving.isClimbing || moving.isClimbCrawling
						|| moving.isCrawlClimbing || moving.isFlying
						|| moving.isSwimming || moving.isDiving
		...
		irp.superRenderRotateCorpse(entityplayer, totalTime, actualRotation,
				f2);
	}

	public void renderLivingAt(AbstractClientPlayer entityplayer, double d,
			double d1, double d2) {
		if (entityplayer instanceof EntityOtherPlayerMP) {
			SmartMoving moving = SmartMovingFactory
					.getOtherSmartMoving(entityplayer.getEntityId());
	...
	public void renderName(AbstractClientPlayer entityPlayer, double d,
			double d1, double d2) {
		boolean changedIsSneaking = false, originalIsSneaking = false;
		if (Minecraft.isGuiEnabled()
				&& entityPlayer != irp.getMovingRenderManager().pointedEntity) {
			SmartMoving moving = SmartMovingFactory.getInstance(entityPlayer);
			if (moving != null) {
	...
	public static void renderGuiIngame(Minecraft minecraft) {
		...
		SmartMovingSelf moving = (SmartMovingSelf) SmartMovingFactory
				.getInstance(minecraft.player);
		if (moving != null && SmartMovingContext.Config.enabled
				&& (SmartMovingContext.Options._displayExhaustionBar.value
			...
			if (minecraft.playerController.shouldDrawHUD()) {
				float maxExhaustion = SmartMovingContext.Client
						.getMaximumExhaustion();
				float exhaustion = Math.min(moving.exhaustion, maxExhaustion);
				boolean drawExhaustion = exhaustion > 0
						&& exhaustion <= maxExhaustion;

				float maxStillJumpCharge = SmartMovingContext.Config._jumpChargeMaximum.value;
				float stillJumpCharge = Math.min(moving.jumpCharge,
						maxStillJumpCharge);

				float maxRunJumpCharge = SmartMovingContext.Config._headJumpChargeMaximum.value;
				float runJumpCharge = Math.min(moving.headJumpCharge,
						maxRunJumpCharge);

				boolean drawJumpCharge = stillJumpCharge > 0
						|| runJumpCharge > 0;
				float maxJumpCharge = stillJumpCharge > runJumpCharge
						? maxStillJumpCharge
						: maxRunJumpCharge;
				...
				if (drawExhaustion || drawJumpCharge) {
					...
					minecraft.getTextureManager()
							.bindTexture(new ResourceLocation("smartmoving",
									"gui/icons.png"));
				...
				if (drawExhaustion) {
					float maxExhaustionForAction = Math
							.min(moving.maxExhaustionForAction, maxExhaustion);
					float maxExhaustionToStartAction = Math.min(
							moving.maxExhaustionToStartAction, maxExhaustion);

					float fitness = maxExhaustion - exhaustion;
					float minFitnessForAction = Float
							.isNaN(maxExhaustionForAction) ? 0
									: maxExhaustion - maxExhaustionForAction;
					float minFitnessToStartAction = Float
							.isNaN(maxExhaustionToStartAction) ? 0
									: maxExhaustion
											- maxExhaustionToStartAction;

					float maxFitnessDrawn = Math.max(
							Math.max(minFitnessToStartAction, fitness),
							minFitnessForAction);

					int halfs = (int) Math
							.floor(maxFitnessDrawn / maxExhaustion * 21F);
					int fulls = halfs / 2;
					int half = halfs % 2;

					int fitnessHalfs = (int) Math
							.floor(fitness / maxExhaustion * 21F);
					int fitnessFulls = fitnessHalfs / 2;
					int fitnessHalf = fitnessHalfs % 2;

					int minFitnessForActionHalfs = (int) Math
							.floor(minFitnessForAction / maxExhaustion * 21F);
					int minFitnessForActionFulls = minFitnessForActionHalfs / 2;
					int minFitnessForActionHalf = minFitnessForActionHalfs % 2;

					int minFitnessToStartActionHalfs = (int) Math.floor(
							minFitnessToStartAction / maxExhaustion * 21F);
					int minFitnessToStartActionFulls = minFitnessToStartActionHalfs
							/ 2;

					_jOffset = height - 39 - 10 - (minecraft.player
							.isInsideOfMaterial(Material.WATER) ? 10 : 0);
				...
				if (drawJumpCharge) {
					...
					int fulls = max ? 10
							: (int) Math.ceil(
									((jumpCharge - 2) * 10D) / maxJumpCharge);
					int half = max ? 0
							: (int) Math
									.ceil((jumpCharge * 10D) / maxJumpCharge)
									- fulls;

					_jOffset = height - 39 - 10
							- (minecraft.player.getTotalArmorValue() > 0 ? 10
									: 0);
	...
	private static void drawIcon(int x, int y) {
		_minecraft.ingameGUI.drawTexturedModalRect(_iOffset, _jOffset, x * 9,
				y * 9, 9, 9);
	}
