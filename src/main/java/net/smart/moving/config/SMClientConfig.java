...
public class SMClientConfig extends SMConfig {
	...
	public boolean isJumpExhaustionEnabled(int speed, int type) {
		...
		if (type == ClimbUp || type == ClimbUpHandsOnly || type == ClimbBackUp || type == ClimbBackUpHandsOnly
	...
	public float getJumpExhaustionGain(int speed, int type, float jumpCharge) {
		...
		if (type == ChargeUp) {
			...
			result += _baseExhautionGainFactor.value * _jumpExhaustionGainFactor.value
					* _upJumpExhaustionGainFactor.value * _jumpChargeExhaustionGainFactor.value
	...
	public float getJumpExhaustionStop(int speed, int type, float jumpCharge) {
		...
		if (type == ChargeUp) {
			...
			result -= _jumpExhaustionStopFactor.value * _upJumpExhaustionStopFactor.value
					* _jumpChargeExhaustionStopFactor.value * Math.min(jumpCharge, _jumpChargeMaximum.value)
	...
	public float getJumpVerticalFactor(int speed, int type) {
		...
		if (type == Angle || type == ClimbUp || type == ClimbUpHandsOnly || type == ClimbBackUp
				|| type == ClimbBackUpHandsOnly || type == ClimbBackHead || type == ClimbBackHeadHandsOnly
	...
	public float getJumpHorizontalFactor(int speed, int type) {
		...
		else if (speed == Standing && type != ClimbBackUp && type != ClimbBackUpHandsOnly && type != ClimbBackHead
	...
	public float getFactor(boolean hunger, boolean onGround, boolean isStanding, boolean isStill, boolean isSneaking,
			boolean isRunning, boolean isSprinting, boolean isClimbing, boolean isClimbCrawling,
			boolean isCeilingClimbing, boolean isDipping, boolean isSwimming, boolean isDiving, boolean isCrawling,
