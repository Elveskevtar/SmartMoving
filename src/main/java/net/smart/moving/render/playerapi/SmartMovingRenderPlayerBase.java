...
public class SmartMovingRenderPlayerBase extends RenderPlayerBase
	...
	public void doRender(AbstractClientPlayer entityplayer, double d, double d1,
		...
		getRenderModel().doRender(entityplayer, d, d1, d2, f,
				renderPartialTicks);
	}
	...
	public void rotateCorpse(AbstractClientPlayer entityplayer, float totalTime,
		...
		getRenderModel().rotateCorpse(entityplayer, totalTime, actualRotation,
				f2);
	}
	...
	public IModelPlayer getMovingModelBipedMain() {
		for (Object layer : renderPlayerAPI.getLayerRenderersField())
			if (layer instanceof LayerArmorBase)
				return SmartMoving.getPlayerBase(
						(net.minecraft.client.model.ModelBiped) Reflect
								.GetField(_modelArmor, layer));
		return null;
	}

	@Override
	public IModelPlayer[] getMovingModels() {
		net.minecraft.client.model.ModelBiped[] modelPlayers = api.player.model.ModelPlayerAPI
				.getAllInstances();
