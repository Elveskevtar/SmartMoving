...
public class SmartMovingServerOptions {
	...
	public SmartMovingServerOptions(SmartMovingConfig config, File optionsPath,
			int gameType) {
		this.config = config;
		...
		config.setCurrentKey(configKey != null && !configKey.value.isEmpty()
				? configKey.value
				: null);
	...
	private static void logConfigState(SmartMovingConfig config,
			String username, boolean reconfig) {
		...
		if (config._globalConfig.value) {
			...
			if (config.enabled) {
				...
				if (currentKey == null)
					FMLLog.info(
							message + "default server configuration" + postfix);
				else {
					String configName = config._configKeyName.value;

					message += "server configuration ";
					if (configName.isEmpty())
						FMLLog.info(message + "with key \"" + currentKey + "\""
								+ postfix);
					else
						FMLLog.info(
								message + "\"" + configName + "\"" + postfix);
				}
			} else
	...
	private static void logSpeedState(SmartMovingConfig config,
			String username) {
		FMLLog.info("Smart Moving speed set to " + config.getSpeedPercent()
				+ "%" + getPostfix(username));
	}
	...
	public String[] writeToProperties(IEntityPlayerMP mp, String key) {
		if (key == null ? !config.enabled
				: key == SmartMovingProperties.Disabled)
			return new String[] { config._globalConfig.getCurrentKey(),
					config._globalConfig.getValueString() };

		Properties properties = new Properties();
		config.write(properties, key);

		String[] result = new String[properties.size() * 2];
		Iterator<Map.Entry<Object, Object>> keys = properties.entrySet()
				.iterator();

		String speedUserExponentKey = mp != null
				? config._speedUserExponent.getCurrentKey()
				: null;
		int i = 0;
		while (keys.hasNext()) {
			...
			if (mp != null && propertyKey.equals(speedUserExponentKey)) {
				Integer userExponent = config._speedUsersExponents.value
						.get(mp.getUsername());
				if (userExponent != null)
					entry.setValue(config._speedUserExponent
							.getValueString(userExponent));
			}
	...
	public synchronized void setPlayerSpeedExponent(IEntityPlayerMP player,
			Integer exponent) {
	...
	public synchronized void setPlayerConfigurationKey(IEntityPlayerMP player,
			String key) {
