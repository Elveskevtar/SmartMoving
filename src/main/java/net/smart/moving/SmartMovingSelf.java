...
public class SmartMovingSelf extends SmartMoving implements ISmartMovingSelf {
	...
	private static SmartMovingOptions Options = SmartMovingContext.Options;
	private static SmartMovingClientConfig Config = SmartMovingContext.Config;
	...
	private final Potion jumpBoost = Potion.REGISTRY
			.getObject(new ResourceLocation("jump_boost"));

	public SmartMovingSelf(EntityPlayer sp, SmartMovingPlayerBase isp) {
		super(sp, isp);
	...
	private void superMoveEntityWithHeading(float moveStrafing,
			float moveForward) {
		...
		boolean isLiquidClimbing = Config.isFreeClimbingEnabled()
				&& sp.fallDistance <= 3.0 && wantClimbUp
				&& sp.collidedHorizontally && !isDiving;
		boolean handledSwimming = handleSwimming(moveForward, moveStrafing,
				speedFactor, wasSwimming, wasDiving, isLiquidClimbing,
				wasJumpingOutOfWater);
		boolean handledLava = handleLava(moveForward, moveStrafing,
				handledSwimming, isLiquidClimbing);
		boolean handledAlternativeFlying = handleAlternativeFlying(moveForward,
				moveStrafing, speedFactor, handledSwimming, handledLava);
		handleLand(moveForward, moveStrafing, speedFactor, handledSwimming,
				handledLava, handledAlternativeFlying, wasShortInWater,
				wasClimbing, wasCeilingClimbing);
	...
	private float getSpeedFactor() {
		return Config.enabled
				? Config._speedFactor.value * Config.getUserSpeedFactor()
						* getLandMovementFactor() * 10F
	...
	private float getSpeedFactor(float moveForward, float moveStrafing) {
		...
		if (isClimbing)
			...
			else if (wantClimbDown && isNeighborClimbing && !(Math
					.abs(sp.posX - lastHorizontalCollisionX) < 0.05
					&& Math.abs(sp.posZ - lastHorizontalCollisionZ) < 0.05)) {
				moveForward = ClimbPullMotion;
				if (isVineOnlyClimbing) {
					if (handsEdgeMeta != Orientation.VineFrontMeta
							&& feetEdgeMeta != Orientation.VineFrontMeta)
						moveForward = 0F;
					else {
						Orientation orientation = Orientation.getOrientation(sp,
								45F, true, false);
						if (orientation != null) {
							float gap = (float) orientation
									.getHorizontalBorderGap(sp);
							float minGap = sp.width / 2;
							float factor = Math.max(0,
									gap * (1 + minGap) - minGap);
	...
	private boolean handleSwimming(float moveForward, float moveStrafing,
			float speedFactor, boolean wasSwimming, boolean wasDiving,
			boolean isLiquidClimbing, boolean wasJumpingOutOfWater) {
		...
		if (handleSwimming) {
			...
			boolean useStandard = !Config.isSwimmingEnabled()
					&& !Config.isDivingEnabled();
			if (sp.isRiding()) {
				resetSwimming();
				useStandard = true;
			}
			...
			if (!useStandard) {
				...
				double totalSwimWaterBorder = getMaxPlayerLiquidBetween(
						getBoundingBox().maxY - 1.8,
						getBoundingBox().maxY + 1.2);
				double minPlayerSwimWaterCeiling = getMinPlayerSolidBetween(
						getBoundingBox().maxY - 1.8,
						getBoundingBox().maxY + 1.2, 0);
				double realTotalSwimWaterBorder = Math.min(totalSwimWaterBorder,
						minPlayerSwimWaterCeiling);
				double minPlayerSwimWaterDepth = totalSwimWaterBorder
						- getMaxPlayerSolidBetween(totalSwimWaterBorder - 2,
								totalSwimWaterBorder, 0);
				double realMinPlayerSwimWaterDepth = totalSwimWaterBorder
						- getMaxPlayerSolidBetween(realTotalSwimWaterBorder - 2,
								realTotalSwimWaterBorder, 0);
				double playerSwimWaterBorder = totalSwimWaterBorder - j
						- j_offset;

				if (isCrawling
						&& playerSwimWaterBorder > SwimCrawlWaterTopBorder)
					standupIfPossible();

				double motionYDiff = 0;
				boolean couldStandUp = playerSwimWaterBorder >= 0
						&& minPlayerSwimWaterDepth <= 1.5;

				boolean diveUp = isp.getIsJumpingField();
				boolean diveDown = esp.movementInput.sneak
						&& Config._diveDownOnSneak.value;
				boolean swimDown = esp.movementInput.sneak
						&& Config._swimDownOnSneak.value;

				boolean wantShallowSwim = couldStandUp
						&& (wasSwimming || wasDiving);
				if (wantShallowSwim) {
					HashSet<Orientation> orientations = Orientation
							.getClimbingOrientations(sp, true, true);
					Iterator<Orientation> iterator = orientations.iterator();
					while (iterator.hasNext())
						if (!(wantShallowSwim &= !iterator.next()
								.isTunnelAhead(sp.world, i, j, k)))
							break;
				}
				...
				if (isCrawling || isClimbCrawling || isCrawlClimbing)
					isDipping = true;
				else if (playerSwimWaterBorder >= 0
						&& playerSwimWaterBorder <= 2) {
					double offset = playerSwimWaterBorder + 0.1625D; // for fine
																		// tuning
					boolean moveSwim = sp.rotationPitch < 0F
					...
					if (diveUp || moveSwim || wantShallowSwim) {
						if (offset < 1.4) {
							...
							if (swimDown)
								motionYDiff = -0.05D
										* (isFast ? Config._sprintFactor.value
												: 1F);
							else if (offset < 1.5)
								motionYDiff = -0.02D;
							if (offset < 1.6)
					...
					diving = true;
					if (diveUp)
						if (isFast && playerSwimWaterBorder < 2.5
								&& isAirBlock(i, j + 3, k))
				...
				if ((isCrawling || isSliding)
						&& playerCrawlWaterBorder < SwimCrawlWaterMaxBorder)
				...
				if (!handleSwimmingRejected) {
					swimming = !useStandard && swimming
							&& Config.isSwimmingEnabled();
					diving = !useStandard && diving && Config.isDivingEnabled();
					dipping = !useStandard && dipping
							&& Config.isSwimmingEnabled();
					...
					if (!useStandard) {
						...
						boolean levitating = diving && !diveUp && !diveDown
								&& moveStrafing == 0F && moveForward == 0F;
						...
						boolean wantJumpOutOfWater = (moveForward != 0
								|| moveStrafing != 0) && sp.collidedHorizontally
								&& diveUp && !isSlow;
						isJumpingOutOfWater = wantJumpOutOfWater
								&& (waterMovementTicks > 10 || sp.onGround
										|| wasJumpingOutOfWater);
						...
						if (diving) {
							if (diveUp || diveDown || levitating)
								...
								moveFlying((float) motionYDiff, moveStrafing,
										moveForward, 0.02F * speedFactor,
						...
						isSwimming = swimming;
						isShallowDiveOrSwim = couldStandUp
								&& (isDiving || isSwimming);
						isDipping = dipping;
						...
						if (isShallowDiveOrSwim
								&& realMinPlayerSwimWaterDepth < SwimCrawlWaterBottomBorder) {
							if (isSlow) {
								...
								sp.move(MoverType.SELF, 0,
										getMaxPlayerSolidBetween(
												getBoundingBox().minY,
												getBoundingBox().maxY, 0)
												- getBoundingBox().minY,
										0);
								isCrawling = false;
								isDiving = false;
						...
						if (moveFlying)
							moveFlying(0f, moveStrafing, moveForward,
									0.02F * speedFactor, false);
						sp.move(MoverType.SELF, sp.motionX, sp.motionY,
								sp.motionZ);
					}
				}
			} else {
			...
			if (useStandard) {
				...
				sp.motionY -= 0.02D;
				if (sp.collidedHorizontally && sp.isOffsetPositionInLiquid(
						sp.motionX,
						((sp.motionY + 0.60000002384185791D) - sp.posY) + dY,
						sp.motionZ)) {
					sp.motionY = 0.30000001192092896D;
				}
			}
	...
	private boolean handleLava(float moveForward, float moveStrafing,
		...
		if (handleLava) {
			...
			if (sp.collidedHorizontally && sp.isOffsetPositionInLiquid(
					sp.motionX,
					((sp.motionY + 0.60000002384185791D) - sp.posY) + d1,
					sp.motionZ)) {
				sp.motionY = 0.30000001192092896D;
			}
		}
	...
	private boolean handleAlternativeFlying(float moveForward,
			float moveStrafing, float speedFactor, boolean handledSwimming,
			boolean handledLava) {
	...
	private void handleLand(float moveForward, float moveStrafing,
			float speedFactor, boolean handledSwimming, boolean handledLava,
		...
		if (!handledSwimming && !handledLava && !handledAlternativeFlying) {
			...
			float horizontalDamping = landMotion(moveForward, moveStrafing,
					speedFactor, isOnLadder, isOnVine);
			boolean crawlingThroughWeb = (isCrawling || isCrawlClimbing)
					&& isp.getIsInWebField();
	...
	private float landMotion(float moveForward, float moveStrafing,
			float speedFactor, boolean isOnLadder, boolean isOnVine) {
		float horizontalDamping;
		final boolean isUsingItem = sp.getItemInUseCount() > 0;
		if (sp.onGround && !isJumping) {
			BlockPos block_pos = new BlockPos(MathHelper.floor(sp.posX),
					MathHelper.floor(getBoundingBox().minY) - 1,
					MathHelper.floor(sp.posZ));
			Block block = getBlock(block_pos.getX(), block_pos.getY(),
					block_pos.getZ());
			if (block != null)
				horizontalDamping = block.getSlipperiness(
						sp.world.getBlockState(block_pos), sp.world, block_pos,
						sp) * HorizontalAirDamping;
			else
				horizontalDamping = HorizontalGroundDamping;

			if (esp.movementInput.jump && isFast
					&& Config.isJumpingEnabled(Config.Sprinting, Config.Up))
		...
		if (isClimbing && climbingUpIsBlockedByLadder())
			...
			else if (Config.enabled && !sp.onGround && !sp.capabilities.isFlying
					&& !isFlying)
				speedFactor *= Config._jumpControlFactor.value;

			float f3 = 0.1627714F / (horizontalDamping * horizontalDamping
					* horizontalDamping);
			float f4 = sp.onGround ? getLandMovementFactor() * f3
					: sp.jumpMovementFactor;
			float rawSpeed = sp.isSprinting() ? f4 / 1.3F : f4;
			if (Config.isRunningEnabled() && isRunning() && !isFast)
				speedFactor *= Config._runFactor.value;

			moveFlying(0F, moveStrafing, moveForward, rawSpeed * speedFactor,
					false);
		}
		...
		if (sp.onGround && !isJumping) {
			...
			if (block != null) {
				float slipperiness = block.getSlipperiness(
						...
						sp);
				if (isSliding) {
					horizontalDamping = 1F / (((1F / slipperiness) - 1F) / 25F
							* Config._slideSlipperinessFactor.value + 1F)
							* 0.98F;
					if (moveStrafing != 0
							&& Config._slideControlDegrees.value > 0) {
						double angle = -Math.atan(sp.motionX / sp.motionZ);
						if (!Double.isNaN(angle)) {
							if (sp.motionZ < 0)
								angle += Math.PI;

							angle -= Config._slideControlDegrees.value
									/ RadiantToAngle
									* Math.signum(moveStrafing);

							double hMotion = Math.sqrt(sp.motionX * sp.motionX
									+ sp.motionZ * sp.motionZ);
		...
		if (isOnLadder || isOnVine) {
			...
			boolean notTotalFreeClimbing = !isClimbing && isOnLadder
					&& !Config.isTotalFreeLadderClimb()
			...
			if (Config.isFreeBaseClimb()) {
				if (isUsingItem || esp.movementInput.sneak && sp.motionY < 0.0D
						&& !sp.onGround && notTotalFreeClimbing) {
					sp.motionY = 0.0D;
				}
			} else {
	...
	private void handleClimbing(boolean isOnLadder, boolean isOnVine,
			boolean wasClimbing) {
		resetClimbing();

		boolean isOnLadderOrVine = isOnLadder || isOnVine;

		if (Config.isStandardBaseClimb() && sp.collidedHorizontally
				&& isOnLadderOrVine) {
			sp.motionY = 0.2 * getSpeedFactor();
		}

		if (Config.isSimpleBaseClimb() && sp.collidedHorizontally
				&& isOnLadderOrVine) {
			int i = MathHelper.floor(sp.posX);
			int j = MathHelper.floor(getBoundingBox().minY);
			int k = MathHelper.floor(sp.posZ);

			boolean feet = Orientation.isClimbable(sp.world, i, j, k);
			boolean hands = Orientation.isClimbable(sp.world, i, j + 1, k);
		...
		if (Config.isSmartBaseClimb() || Config.isFreeClimbingEnabled()) {
			...
			if (Config.isSmartBaseClimb() && isOnLadderOrVine
					&& sp.collidedHorizontally) {
				boolean feet = Orientation.isClimbable(sp.world, i, j, k);
				boolean hands = Orientation.isClimbable(sp.world, i, j + 1, k);
				...
				if (feet && hands)
					...
					boolean handsSubstitute = Orientation.PZ
							.isHandsLadderSubstitute(sp.world, i, j + 1, k)
							|| Orientation.NZ.isHandsLadderSubstitute(sp.world,
									i, j + 1, k)
							|| Orientation.ZP.isHandsLadderSubstitute(sp.world,
									i, j + 1, k)
							|| Orientation.ZN.isHandsLadderSubstitute(sp.world,
									i, j + 1, k);
					...
					boolean feetSubstitute = Orientation.ZZ
							.isFeetLadderSubstitute(sp.world, i, j, k)
							|| Orientation.PZ.isFeetLadderSubstitute(sp.world,
									i, j, k)
							|| Orientation.NZ.isFeetLadderSubstitute(sp.world,
									i, j, k)
							|| Orientation.ZP.isFeetLadderSubstitute(sp.world,
									i, j, k)
							|| Orientation.ZN.isFeetLadderSubstitute(sp.world,
									i, j, k);
			...
			if (Config.isFreeClimbingEnabled()
					&& sp.fallDistance <= Config._freeClimbFallMaximumDistance.value
					&& (!isOnLadderOrVine || Config.isFreeBaseClimb())) {
				boolean exhaustionAllowsClimbing = !Config
						.isClimbExhaustionEnabled()
						|| (exhaustion <= Config._climbExhaustionStop.value
								&& (wasClimbing
										|| exhaustion <= Config._climbExhaustionStart.value));

				boolean preferClimb = false;
				if (wantClimbUp || wantClimbDown) {
					if (Config.isClimbExhaustionEnabled()) {
						maxExhaustionForAction = Math.min(
								maxExhaustionForAction,
								Config._climbExhaustionStop.value);
						maxExhaustionToStartAction = Math.min(
								maxExhaustionToStartAction,
				...
				if (preferClimb) {
					...
					Orientation.PZ.seekClimbGap(rotation, sp.world, i, id, jh,
							k, kd, isClimbCrawling, isCrawlClimbing,
							isSmallClimbing, inout_handsClimbing,
							inout_feetClimbing, out_handsClimbGap,
							out_feetClimbGap);
					Orientation.NZ.seekClimbGap(rotation, sp.world, i, id, jh,
							k, kd, isClimbCrawling, isCrawlClimbing,
							isSmallClimbing, inout_handsClimbing,
							inout_feetClimbing, out_handsClimbGap,
							out_feetClimbGap);
					Orientation.ZP.seekClimbGap(rotation, sp.world, i, id, jh,
							k, kd, isClimbCrawling, isCrawlClimbing,
							isSmallClimbing, inout_handsClimbing,
							inout_feetClimbing, out_handsClimbGap,
							out_feetClimbGap);
					Orientation.ZN.seekClimbGap(rotation, sp.world, i, id, jh,
							k, kd, isClimbCrawling, isCrawlClimbing,
							isSmallClimbing, inout_handsClimbing,
							inout_feetClimbing, out_handsClimbGap,
							out_feetClimbGap);
					...
					isNeighborClimbing = handsClimbing != HandsClimbing.None
							|| feetClimbing != FeetClimbing.None;
					hasNeighborClimbGap = out_handsClimbGap.CanStand
							|| out_feetClimbGap.CanStand;
					hasNeighborClimbCrawlGap = out_handsClimbGap.MustCrawl
							|| out_feetClimbGap.MustCrawl;

					if (!isSmallClimbing) {
						Orientation.PP.seekClimbGap(rotation, sp.world, i, id,
								jh, k, kd, isClimbCrawling, isCrawlClimbing,
								isSmallClimbing, inout_handsClimbing,
								inout_feetClimbing, out_handsClimbGap,
								out_feetClimbGap);
						Orientation.NP.seekClimbGap(rotation, sp.world, i, id,
								jh, k, kd, isClimbCrawling, isCrawlClimbing,
								isSmallClimbing, inout_handsClimbing,
								inout_feetClimbing, out_handsClimbGap,
								out_feetClimbGap);
						Orientation.NN.seekClimbGap(rotation, sp.world, i, id,
								jh, k, kd, isClimbCrawling, isCrawlClimbing,
								isSmallClimbing, inout_handsClimbing,
								inout_feetClimbing, out_handsClimbGap,
								out_feetClimbGap);
						Orientation.PN.seekClimbGap(rotation, sp.world, i, id,
								jh, k, kd, isClimbCrawling, isCrawlClimbing,
								isSmallClimbing, inout_handsClimbing,
								inout_feetClimbing, out_handsClimbGap,
								out_feetClimbGap);
					...
					hasClimbGap = out_handsClimbGap.CanStand
							|| out_feetClimbGap.CanStand;
					hasClimbCrawlGap = out_handsClimbGap.MustCrawl
							|| out_feetClimbGap.MustCrawl;

					if (handsClimbing == HandsClimbing.BottomHold
							&& Orientation.isLadder(getState(i, j + 2, k))) {
						Orientation ladderOrientation = Orientation
								.getKnownLadderOrientation(sp.world, i, j + 2,
										k);
						int remote_i = i + ladderOrientation._i;
						int remote_k = k + ladderOrientation._k;
						if (!getState(remote_i, j, remote_k).getMaterial()
								.isSolid()
								&& !getState(remote_i, j + 1, remote_k)
										.getMaterial().isSolid())
							handsClimbing = HandsClimbing.None;
					}

					if (!grabButton.Pressed && handsClimbing == HandsClimbing.Up
							&& feetClimbing == FeetClimbing.None) {
						if (!sp.collidedHorizontally && isAirBlock(i, j, k)
								&& isAirBlock(i, j + 1, k))
							handsClimbing = HandsClimbing.None;
					}
					...
					if (feetClimbing.IsRelevant()
						...
						if (wantClimbUp) {
							...
							if (feetClimbing == FeetClimbing.FastUp
									&& !(handsClimbing == HandsClimbing.None
											&& sp.onGround
											&& out_feetClimbGap.Block != Block
													.getBlockFromName("bed"))) {
								// climbing fast
								setShouldClimbSpeed(FastUpMotion,
										HandsClimbing.NoGrab,
								...
								setShouldClimbSpeed(
									...
									&& !(handsClimbing == HandsClimbing.TopHold
											&& feetClimbing == FeetClimbing.TopWithHands)) {
								// climbing all limbed
								setShouldClimbSpeed(MediumUpMotion, (hasClimbGap
										|| hasClimbCrawlGap)
										&& !(handsClimbing == HandsClimbing.Sink
												&& feetClimbing == FeetClimbing.BaseWithHands)
														? HandsClimbing.MiddleGrab
								...
								if (!jumpButton.StartPressed
										|| !(isClimbJumping = tryJump(
												feetClimbing != FeetClimbing.None
														? Config.ClimbUp
														: Config.ClimbUpHandsOnly,
												null, null, null))) {
									if (handsClimbing == HandsClimbing.Sink
											&& feetClimbing == FeetClimbing.BaseHold
											|| handsClimbing == HandsClimbing.TopHold
													&& feetClimbing == FeetClimbing.TopWithHands)
							...
							if (isClimbHolding) {
								...
								if (jumpButton.StartPressed) {
									...
									int type = (Options._climbJumpBackHeadOnGrab.value
											...
											: !grabButton.Pressed)
													? (handsOnly
															? Config.ClimbBackHead
															: Config.ClimbBackHeadHandsOnly)
													: (handsOnly
															? Config.ClimbBackUp
															: Config.ClimbBackUpHandsOnly);
						...
						if (isClimbing)
							handleCrash(
									Config._freeClimbFallDamageStartDistance.value,
				...
				if (SmartMovingOptions.hasRedPowerWire && !isClimbing
						&& wantClimbUp && wasClimbing) // to climb up RedPower
														// wire bottom covers
					sp.motionY = 0.15;

				isHandsVineClimbing = isClimbing
						&& handsEdgeBlock == Block.getBlockFromName("vine");
				isFeetVineClimbing = isClimbing
						&& feetEdgeBlock == Block.getBlockFromName("vine");

				isVineAnyClimbing = isHandsVineClimbing || isFeetVineClimbing;

				isVineOnlyClimbing = isVineAnyClimbing
						&& !(handsEdgeBlock != null
								&& handsEdgeBlock != Block
										.getBlockFromName("vine")
								|| feetEdgeBlock != null
										&& feetEdgeBlock != Block
												.getBlockFromName("vine"));
			}
	...
	private void handleCeilingClimbing(boolean wasCeilingClimbing) {
		boolean exhaustionAllowsClimbCeiling = !Config
				.isCeilingClimbExhaustionEnabled()
				|| (exhaustion <= Config._ceilingClimbExhaustionStop.value
						&& (wasCeilingClimbing
								|| exhaustion <= Config._ceilingClimbExhaustionStart.value));

		boolean climbCeilingCrawlingStartConflict = !Config
				.isFreeClimbingEnabled() && isCrawling && !wasCrawling;
		boolean couldClimbCeiling = wantClimbCeiling && !isClimbing
				&& (!isCrawling || climbCeilingCrawlingStartConflict)
				&& !isCrawlClimbing;
		if (couldClimbCeiling && Config.isCeilingClimbExhaustionEnabled()) {
			maxExhaustionForAction = Math.min(maxExhaustionForAction,
					Config._ceilingClimbExhaustionStop.value);
		...
		if (couldClimbCeiling && exhaustionAllowsClimbCeiling) {
			double id = sp.posX;
			double jd = getBoundingBox().maxY
					+ (climbCeilingCrawlingStartConflict ? 1F : 0F);
			double kd = sp.posZ;
			...
			if (topCeilingClimbing || bottomCeilingClimbing) {
				...
				double actuallySolidHeight = getMinPlayerSolidBetween(jd,
						jd + 0.6, 0.2);
				if (jgap < 1.9 && actuallySolidHeight < jd + 0.5) {
					...
					handsEdgeBlock = topCeilingClimbing ? topBlock
							: bottomBlock;
				}
			}
		}
	...
	private void handleExhaustion(double diffX, double diffY, double diffZ) {
		...
		if (Config.enabled) {
			...
			if (!sp.isRiding()) {
				float horizontalMovement = MathHelper
						.sqrt(diffX * diffX + diffZ * diffZ);
				float movement = MathHelper
						.sqrt(horizontalMovement * horizontalMovement
								+ diffY * diffY);

				int relevantMovementFactor = Math.round(movement * 100F);

				if (Config.isHungerGainEnabled()) {
					float hungerGainFactor = Config.getFactor(true, sp.onGround,
							isStanding, isStill, isSlow, isRunning, isFast,
							isClimbing, isClimbCrawling, isCeilingClimbing,
							isDipping, isSwimming, isDiving, isCrawling,
							isCrawlClimbing);
					hungerIncrease += Config._alwaysHungerGain.value
							+ relevantMovementFactor * 0.0001F
									* hungerGainFactor;
				}

				float additionalExhaustion = 0F;
				if (isClimbing && !isStill
				...
				if (isCeilingClimbing && !isStanding
				...
				if (foreignExhaustionFactor > 0) {
					additionalExhaustion += foreignExhaustionFactor
							* Config._baseExhautionGainFactor.value;

					if (foreignMaxExhaustionForAction == Float.MAX_VALUE)
						foreignMaxExhaustionForAction = SmartMovingContext.Client
								.getMaximumExhaustion();
					maxExhaustionForAction = Math.min(maxExhaustionForAction,
							foreignMaxExhaustionForAction);

					if (foreignMaxExhaustionToStartAction == Float.MAX_VALUE)
						foreignMaxExhaustionToStartAction = SmartMovingContext.Client
								.getMaximumExhaustion();
					maxExhaustionToStartAction = Math.min(
							maxExhaustionToStartAction,
			...
			if (exhaustion > 0) {
				boolean exhaustionLossPossible = !Config
						.isExhaustionLossHungerEnabled()
						|| sp.getFoodStats()
								.getFoodLevel() > Config._exhaustionLossFoodLevelMinimum.value;
				if (exhaustionLossPossible) {
					float exhaustionLossFactor = Config.getFactor(false,
							sp.onGround, isStanding, isStill, isSlow, isRunning,
							isFast, isClimbing, isClimbCrawling,
							isCeilingClimbing, isDipping, isSwimming, isDiving,
					...
					if (Config.isExhaustionLossHungerEnabled())
						hungerIncrease += Config._exhaustionLossHungerFactor.value
								* exhaustionLoss;
				}
			}
		...
		SmartMovingPacketStream.sendHungerChange(SmartMovingComm.instance,
				hungerIncrease);
	}
	...
	public void setMaxExhaustionForAction(float maxExhaustionForAction) {
		if (!Float.isNaN(maxExhaustionForAction) && maxExhaustionForAction >= 0)
			foreignMaxExhaustionForAction = Math
					.min(foreignMaxExhaustionForAction, maxExhaustionForAction);
	}

	@Override
	public void setMaxExhaustionToStartAction(
			float maxExhaustionToStartAction) {
		if (!Float.isNaN(maxExhaustionToStartAction)
				&& maxExhaustionToStartAction >= 0)
			foreignMaxExhaustionToStartAction = Math.min(
					foreignMaxExhaustionToStartAction,
					maxExhaustionToStartAction);
	}
	...
	private void fromSwimmingOrDiving(boolean wasShortInWater) {
		...
		if (wasShortInWater && !isShortInWater && !isp.getSleepingField()) {
			...
			double crawlStandUpBottom = getMaxPlayerSolidBetween(
					getBoundingBox().minY - 1D, getBoundingBox().minY, 0);
			double crawlStandUpLiquidCeiling = getMinPlayerLiquidBetween(
					getBoundingBox().maxY, getBoundingBox().maxY + 1.1D);
			double crawlStandUpCeiling = getMinPlayerSolidBetween(
					getBoundingBox().maxY, getBoundingBox().maxY + 1.1D, 0);
			...
			if (crawlStandUpCeiling - crawlStandUpBottom < sp.height) {
				...
				isDipping = false;
				setHeightOffset(-1F);
			} else if (crawlStandUpLiquidCeiling
					- crawlStandUpBottom < sp.height) {
				...
				if (isSlow
						&& crawlStandUpBottom > getBoundingBox().minY + 0.5D) {
	...
	private void setShouldClimbSpeed(double value, int handsClimbType,
			int feetClimbType) {
	...
	private void setOnlyShouldClimbSpeed(double value) {
		...
		if (value != HoldMotion) {
			...
			if (Config.isFreeBaseClimb() && value == MediumUpMotion)
				switch (getOnLadder(Integer.MAX_VALUE, false,
						isClimbCrawling)) {
				case 1:
			...
			if (value > HoldMotion)
				value = ((value - HoldMotion)
						* Config._freeClimbingUpSpeedFactor.value * factor
						+ HoldMotion);
			else
				value = HoldMotion - (HoldMotion - value)
						* Config._freeClimbingDownSpeedFactor.value * factor;
			...
			if (distanceClimbedModified > nextClimbDistance) {
				...
				if (isClimbing)
					if (handsEdgeBlock == null)
						...
						stepBlock = nextClimbDistance % 2 != 0 ? feetEdgeBlock
								: handsEdgeBlock;
				else
					stepBlock = handsEdgeBlock;
				...
				nextClimbDistance++;
				if (stepBlock != null) {
					SoundType stepsound = stepBlock.getSoundType(
							stepBlock.getDefaultState(), sp.world,
							sp.getPosition(), sp);
					if (stepsound != null)
	...
	private void playSound(SoundEvent soundEvent, float volume, float pitch) {
		sp.world.playSound(sp, new BlockPos(sp.posX, sp.posY, sp.posZ),
				soundEvent, sp.getSoundCategory(), volume, pitch);
		SmartMovingPacketStream.sendSound(SmartMovingComm.instance,
				soundEvent.getSoundName().toString(), volume, pitch);
	}
	...
	private void resetHeightOffset() {
		...
		bb = new AxisAlignedBB(bb.minX, bb.minY + heightOffset, bb.minZ,
	...
	public void beforeOnUpdate() {
		...
		if (sp.world.isRemote && updateCounter < 10) {
			List<?> chatMessageList = (List<?>) Reflect.GetField(
					GuiNewChat.class, isp.getMcField().ingameGUI.getChatGUI(),
					SmartMovingInstall.GuiNewChat_chatMessageList);
			for (int i = 0; i < chatMessageList.size(); i++)
				if (SmartMovingComm
						.processBlockCode(((ChatLine) chatMessageList.get(i))
	...
	public void afterOnUpdate() {
		correctOnUpdate(isSwimming || isDiving || isDipping || isCrawling,
				isSwimming);
		...
		if (Config.enabled) {
			...
			if (isFast || isSprintJump || isRunning()) {
				...
				if (isFast || isSprintJump) {
					...
					perspectiveFactor *= 1.3F
							* Options._perspectiveRunFactor.value;
				}
			}

			if (fadingPerspectiveFactor != -1)
				fadingPerspectiveFactor += (perspectiveFactor
	...
	public void afterOnLivingUpdate() {
		if (Options._flyWhileOnGround.value
				&& !(sneakButton.Pressed && grabButton.Pressed)
				&& wasCapabilitiesIsFlying && !sp.capabilities.isFlying
				&& sp.onGround) {
			sp.cameraYaw = 0;
			sp.prevCameraYaw = 0;
	...
	public void handleJumping() {
		...
		boolean jump = jumpAvoided && sp.onGround && isp.getIsJumpingField()
				&& !sp.isInWater() && !sp.isInLava();
		if (jump) {
			if (getBoundingBox().minY
					- getMaxPlayerSolidBetween(getBoundingBox().minY - 0.2D,
							getBoundingBox().minY, 0) >= 0.01D)
		...
		if (Config.isJumpChargingEnabled()) {
			...
			boolean actualJumpCharging = isJumpChargingPossible
					&& (!Config._jumpChargeCancelOnSneakRelease.value
							|| wouldIsSneaking);
		...
		if (Config.isHeadJumpingEnabled()) {
			isHeadJumpCharging = grabButton.Pressed && (isGroundSprinting
					|| isSprintJump || (isRunning() && sp.onGround))
					&& !isCrawling;
			...
			if (sp.posY - MathHelper.floor(sp.posY) > (isSlow ? 0.37 : 0.6)) {
				...
				if (!isStillSwimmingJump && sp.onGround && jumpCharge == 0) {
					if (tryJump(Config.Up, true, null, null)) {
						Random rand = sp.getRNG();
						playSound("random.splash", 0.05F, 1.0F
								+ (rand.nextFloat() - rand.nextFloat()) * 0.4F);
					}
				}
			}
	...
	public void handleWallJumping() {
		...
		if (grabButton.Pressed) {
			if (sp.fallDistance > Config._wallHeadJumpFallMaximumDistance.value)
				return;
			jumpType = wasCollidedHorizontally ? Config.WallHeadSlide
					: Config.WallHead;
		} else {
			if (sp.fallDistance > Config._wallUpJumpFallMaximumDistance.value)
				return;
			jumpType = wasCollidedHorizontally ? Config.WallUpSlide
					: Config.WallUp;
		}
	...
	public boolean tryJump(int type, Boolean inWaterOrNull,
		...
		boolean isRunning = isRunningOrNull != null ? isRunningOrNull
				: isRunning();
		boolean charged = type == Config.ChargeUp;
		boolean up = type == Config.Up || type == Config.ChargeUp
				|| type == Config.HeadUp || type == Config.ClimbUp
				|| type == Config.ClimbUpHandsOnly || type == Config.ClimbBackUp
				|| type == Config.ClimbBackUpHandsOnly
				|| type == Config.ClimbBackHead
				|| type == Config.ClimbBackHeadHandsOnly || type == Config.Angle
		...
		if (enabled) {
			boolean exhausionEnabled = Config.isJumpExhaustionEnabled(speed,
					type);
			if (exhausionEnabled) {
				float maxExhausionForJump = Config.getJumpExhaustionStop(speed,
						type, jumpCharge);
				if (exhaustion > maxExhausionForJump)
					return false;
				maxExhaustionToStartAction = Math
						.min(maxExhaustionToStartAction, maxExhausionForJump);
				maxExhaustionForAction = Math.min(maxExhaustionForAction,
						maxExhausionForJump + Config.getJumpExhaustionGain(
								speed, type, jumpCharge));
			}
			...
			float horizontalJumpFactor = Config.getJumpHorizontalFactor(speed,
					type) * jumpFactor;
			float verticalJumpFactor = Config.getJumpVerticalFactor(speed, type)
					* jumpFactor;
			float jumpChargeFactor = charged
					? Config.getJumpChargeFactor(jumpCharge)
					: 1F;

			if (!up) {
				horizontalJumpFactor = MathHelper
						.sqrt(horizontalJumpFactor * horizontalJumpFactor
								+ verticalJumpFactor * verticalJumpFactor);
				verticalJumpFactor = 0;
			}

			Double maxHorizontalMotion = null;
			double horizontalMotion = MathHelper.sqrt(
					jumpMotionX * jumpMotionX + jumpMotionZ * jumpMotionZ);
			double verticalMotion = -0.078
					+ 0.498 * verticalJumpFactor * jumpChargeFactor;

			if (horizontalJumpFactor > 1F && !sp.collidedHorizontally)
				maxHorizontalMotion = (double) Config
						.getMaxHorizontalMotion(speed, type, inWater)
						* getSpeedFactor();

			if (head) {
				double normalAngle = Math
						.atan(verticalMotion / horizontalMotion);
				double totalMotion = Math.sqrt(verticalMotion * verticalMotion
						+ horizontalMotion * horizontalMotion);

				double newAngle = Config.getHeadJumpFactor(headJumpCharge)
						* normalAngle;
				...
				if (maxHorizontalMotion != null)
					maxHorizontalMotion = maxHorizontalMotion
							* (newHorizontalMotion / horizontalMotion);
			...
			if (angle != null) {
				...
				boolean reset = type == Config.WallUp
						|| type == Config.WallHead;

				double horizontal = Math.max(horizontalMotion,
						horizontalJumpFactor);
				double moveX = -Math.sin(jumpAngle);
				double moveZ = Math.cos(jumpAngle);

				sp.motionX = getJumpMoving(jumpMotionX, moveX, reset,
						horizontal, horizontalJumpFactor);
				sp.motionZ = getJumpMoving(jumpMotionZ, moveZ, reset,
						horizontal, horizontalJumpFactor);
			...
			if (horizontalMotion > 0) {
				double absoluteMotionX = Math.abs(sp.motionX)
						* horizontalJumpFactor;
				double absoluteMotionZ = Math.abs(sp.motionZ)
						* horizontalJumpFactor;

				if (maxHorizontalMotion != null) {
					absoluteMotionX = Math.min(absoluteMotionX,
							maxHorizontalMotion * (horizontalJumpFactor
									* (Math.abs(sp.motionX)
											/ horizontalMotion)));
			...
			if (exhausionEnabled) {
				float exhaustionFromJump = Config.getJumpExhaustionGain(speed,
						type, jumpCharge);
	...
	private static double getJumpMoving(double actual, double move,
			boolean reset, double horizontal, float horizontalJumpFactor) {
		if (!reset)
			return actual + move * horizontal;
		else if (Math.signum(actual) != Math.signum(move))
			return move * horizontalJumpFactor;
		else
			return Math.max(Math.abs(actual), Math.abs(move) * horizontal)
					* Math.signum(move);
	}

	private static int getJumpSpeed(boolean isStanding, boolean isSlow,
	...
	private void standupIfPossible(boolean tryLanding,
			boolean restoreFromFlying) {
		if (heightOffset >= 0)
			return;
	...
	private void handleCrash(float fallDamageStartDistance,
			float fallDamageFactor) {
		if (sp.fallDistance >= 2.0F)
			sp.addStat(StatList.FALL_ONE_CM,
					(int) Math.round(sp.fallDistance * 100D));

		if (sp.fallDistance >= fallDamageStartDistance) {
			sp.attackEntityFrom(DamageSource.FALL,
					(int) Math.ceil((sp.fallDistance - fallDamageStartDistance)
							* fallDamageFactor));
	...
	public void updateEntityActionState(boolean startSleeping) {
		...
		if (toggleButton.StartPressed) {
			if (Config == Options)
				...
				SmartMovingPacketStream
						.sendConfigChange(SmartMovingComm.instance);
		}

		if (Config.isUserSpeedEnabled() && !Config.isUserSpeedAlwaysDefault()
				&& (speedIncreaseButton.StartPressed
						|| speedDecreaseButton.StartPressed)) {
			int difference = 0;
			...
			if (difference != 0) {
				if (Config == Options)
					...
					SmartMovingPacketStream.sendSpeedChange(
							SmartMovingComm.instance, difference, null);
			}
		}

		boolean initializeCrawling = false;
		if (!initialized && !(sp.world.isRemote && multiPlayerInitialized != 0)
				&& !sp.isRiding()) {
			if (getMaxPlayerSolidBetween(getBoundingBox().minY,
					getBoundingBox().maxY, 0) > getBoundingBox().minY) {
		...
		if (!startSleeping) {
			...
			isp.setIsJumpingField(esp.movementInput.jump && !isCrawling
					&& !isSliding
					&& !(Config.isHeadJumpingEnabled() && grabButton.Pressed
							&& sp.isSprinting())
					&& !(Config.isJumpChargingEnabled() && wouldIsSneaking
							&& sp.onGround && isStanding)
					&& !blockJumpTillButtonRelease);
		}

		boolean isSleeping = isp.getSleepingField();
		boolean disabled = !Config.enabled || sp.isRiding() || isSleeping
				|| startSleeping;
		...
		double horizontalSpeedSquare = sp.motionX * sp.motionX
				+ sp.motionZ * sp.motionZ;
		// double verticalSpeedSquare = (sp.motionY + HoldMotion) * (sp.motionY
		// + HoldMotion);
		// double speedSquare = horizontalSpeedSquare + verticalSpeedSquare;

		boolean blocked = minecraft.currentScreen != null
				&& !minecraft.currentScreen.allowUserInput;
		...
		if (isCrawling || isClimbCrawling) {
			crawlStandUpBottom = getMaxPlayerSolidBetween(
					getBoundingBox().minY - (initializeCrawling ? 0D : 1D),
					getBoundingBox().minY,
					Config._crawlOverEdge.value ? 0 : -0.05);
			double crawlStandUpCeiling = getMinPlayerSolidBetween(
					getBoundingBox().maxY, getBoundingBox().maxY + 1.1D, 0);
			mustCrawl = crawlStandUpCeiling - crawlStandUpBottom < sp.height
					- heightOffset;
		}

		if (esp.capabilities.isFlying && (Config.isFlyingEnabled()
				|| Config.isLevitateSmallEnabled()))
			mustCrawl = false;

		boolean inputContinueCrawl = Options.isCrawlToggleEnabled()
				? crawlToggled
				: sneakButton.Pressed || !Config.isFreeClimbingEnabled()
						&& grabButton.Pressed;
		if (contextContinueCrawl) {
			if (inputContinueCrawl || sp.isInWater() || mustCrawl)
				...
				double crawlStandUpLiquidCeiling = getMinPlayerLiquidBetween(
						getBoundingBox().maxY, getBoundingBox().maxY + 1.1D);
				if (crawlStandUpLiquidCeiling - crawlStandUpBottom >= sp.height
						+ 1F)
					contextContinueCrawl = false;
			}
		}
		...
		boolean canCrawl = !isSwimming && !isDiving && (!isDipping
				|| (dippingDepth + heightOffset) < SwimCrawlWaterTopBorder)
				&& !isClimbing
		...
		wantCrawlNotClimb = (wantCrawlNotClimb
				...
				&& isCrawling && sp.collidedHorizontally;
		...
		boolean wouldWantClimb = (grabButton.Pressed
				...
				|| (Config.isFreeClimbAutoLaddderEnabled()
						&& isFacedToLadder(isClimbCrawling))
				|| (Config.isFreeClimbAutoVineEnabled() && isFacedToSolidVine))
				&& (!isSliding || grabButton.Pressed
						&& esp.movementInput.moveForward > 0F)
		...
		wantClimbUp = wantClimb && esp.movementInput.moveForward > 0F
				|| (isVineAnyClimbing && jumpButton.Pressed
						...
						&& (!isCrawling || sp.collidedHorizontally)
						&& (!isSliding || sp.collidedHorizontally);

		wantClimbDown = wantClimb && esp.movementInput.moveForward <= 0F
				&& !wantCrawl;

		wantClimbCeiling = Config.isCeilingClimbingEnabled()
				&& grabButton.Pressed && !wantCrawlNotClimb && !isSneaking()
				&& sp.getItemInUseCount() < 1 && !disabled;

		boolean restoreFromFlying = false;

		boolean wasFlying = isFlying;
		isFlying = Config.isFlyingEnabled() && sp.capabilities.isFlying
				&& !isSwimming && !isDiving;
		...
		isHeadJumping = isHeadJumping && !sp.onGround
				&& !(isSwimming || isDiving)
		...
		if (Config.isSlidingEnabled() && grabButton.Pressed
				&& (isGroundSprinting
						|| (wasRunning && !isRunning && sp.onGround))
		...
		if (isSliding && (!sneakButton.Pressed
				|| horizontalSpeedSquare < Config._slidingSpeedStopFactor.value
						* 0.01)) {
			isSliding = false;
			wasCrawling = toCrawling();
		}
		...
		boolean wouldWantSneak = !isFlying && !isSliding && !isHeadJumping
				...
				&& !(isSwimming && Config._swimDownOnSneak.value
						&& !isFakeShallowWaterSneaking)
				&& sneakContinueInput && !wantCrawl && !mustCrawl
				&& (!Config.isCrawlingEnabled() || !grabButton.Pressed);

		boolean wantSneak = Config.isSneakingEnabled() && wouldWantSneak;

		boolean moveButtonPressed = esp.movementInput.moveForward != 0F
				|| esp.movementInput.moveStrafe != 0F;
		boolean moveForwardButtonPressed = esp.movementInput.moveForward > 0F;

		wantSprint = Config.isSprintingEnabled() && !isSliding
				&& sprintButton.Pressed
				&& (moveForwardButtonPressed || isClimbing
						|| (isSwimming
								&& (moveButtonPressed || (sneakButton.Pressed
										&& Config._swimDownOnSneak.value)))
						|| (isDiving && (moveButtonPressed || jumpButton.Pressed
								|| (sneakButton.Pressed
										&& Config._diveDownOnSneak.value)))
						|| (isFlying && (moveButtonPressed || jumpButton.Pressed
								|| sneakButton.Pressed)))
				&& !disabled;

		boolean exhaustionAllowsRunning = !Config.isRunExhaustionEnabled()
				|| (exhaustion < Config._runExhaustionStop.value && (wasRunning
						|| exhaustion < Config._runExhaustionStart.value));

		if (isRunning && sp.onGround && Config.isRunExhaustionEnabled()) {
			maxExhaustionForAction = Math.min(maxExhaustionForAction,
					Config._runExhaustionStop.value);
			maxExhaustionToStartAction = Math.min(maxExhaustionToStartAction,
					Config._runExhaustionStart.value);
		}

		if (!exhaustionAllowsRunning && isRunning)
			sp.setSprinting(isRunning = false);

		if (!sp.onGround && isFast && !isClimbing && !isCeilingClimbing
				&& !isDiving && !isSwimming)
			isSprintJump = true;

		boolean exhaustionAllowsSprinting = !Config.isSprintExhaustionEnabled()
				|| (exhaustion <= Config._sprintExhaustionStop.value && (isFast
						|| isSprintJump
						|| exhaustion <= Config._sprintExhaustionStart.value));
		...
		if (wantSprint && !wantSneak) {
			if (!isSprintJump && Config.isSprintExhaustionEnabled()) {
				maxExhaustionForAction = Math.min(maxExhaustionForAction,
						Config._sprintExhaustionStop.value);
				maxExhaustionToStartAction = Math.min(
						maxExhaustionToStartAction,
						Config._sprintExhaustionStart.value);
			}
		...
		if (isClimbing && preferSprint) {
			...
			if (wantClimbUp)
				minTickDistance = 0.07
						* Config._freeClimbingUpSpeedFactor.value;
			else if (wantClimbDown)
				minTickDistance = 0.11
						* Config._freeClimbingDownSpeedFactor.value;
			else
				minTickDistance = 0.07;

			isClimbSprintSpeed = net.smart.render.statistics.SmartStatisticsFactory
					.getInstance(sp).getTickDistance() >= minTickDistance;
		}

		boolean canAnySprint = preferSprint && !sp.isBurning()
				&& (Config._sprintDuringItemUsage.value
		...
		isGroundSprinting = canHorizontallySprint && sp.onGround && !isSwimming
				&& !isDiving && !isClimbing;
		...
		boolean isClimbSprinting = canAnySprint && isClimbing
				&& isClimbSprintSpeed;

		isFast = isGroundSprinting || isClimbSprinting || isSwimSprinting
				|| isDiveSprinting || isCeilingSprinting || isFlyingSprinting
				|| isClimbSprinting;
		...
		isCrawlClimbing = (wasCrawling || isCrawlClimbing) && isClimbing
				&& isNeighborClimbing && (sneakButton.Pressed || crawlToggled)
				&& esp.movementInput.moveForward > 0F;
		if (isCrawlClimbing) {
			boolean canStandUp = !isPlayerInSolidBetween(
					getBoundingBox().minY - (isClimbCrawling ? 0.95D : 1D),
					getBoundingBox().minY);
			if (canStandUp) {
			...
			if (!isClimbing) {
				...
				if (!toCrawling)
					resetHeightOffset();
				double minY = getBoundingBox().minY;
				move(0, (-minY + Math.floor(minY) + (toCrawling ? 0F : 1F)), 0,
						true);
			} else if (!toCrawling) {
				resetHeightOffset();
				double minY = getBoundingBox().minY;
		...
		boolean needClimbCrawling = hasClimbCrawlGap
				|| (hasClimbGap && isClimbHolding);
		...
		isClimbCrawling = canClimbCrawling
				&& ((needClimbCrawling && climbIntoCount == 0)
						|| climbIntoCount > 1);
		if (isClimbCrawling && !wasClimbCrawling) {
			...
			if (mustCrawl || sneakButton.Pressed || crawlToggled) {
				double gapUnderneight = getBoundingBox().minY
						- getMaxPlayerSolidBetween(getBoundingBox().minY - 1D,
								getBoundingBox().minY, 0);
		...
		if (grabButton.StartPressed)
			if (isShallowDiveOrSwim && wouldWantClimb) {
				...
				// water
				resetHeightOffset();
				move(0, (getMaxPlayerSolidBetween(getBoundingBox().minY,
						getBoundingBox().maxY, 0) - getBoundingBox().minY), 0,
						true);
				if (jumpButton.Pressed)
					isStillSwimmingJump = true;
			} else if (isDipping && wouldWantCrawl
					&& dippingDepth >= SwimCrawlWaterBottomBorder)
		...
		if (continueWallJumping
				&& (sp.onGround || isClimbing || !jumpButton.Pressed))
		...
		wantWallJumping = canWallJumping && (triggerWallJumping
				|| continueWallJumping || (wantWallJumping && jumpButton.Pressed
						&& !sp.collidedHorizontally));

		boolean canAngleJump = !isSleeping && sp.onGround && !isCrawling
				&& !isClimbing && !isClimbCrawling && !isSwimming && !isDiving;
		...
		if (isSneakToggleEnabled || isCrawlToggleEnabled) {
			...
			if (isCrawling && sneakButton.StopPressed
					&& !ignoreNextStopSneakButtonPressed)
				willStopCrawlStartSneak = true;
			if (!isCrawling && !isCrawlClimbing && !isClimbCrawling)
				willStopCrawl = true;
		...
		if (isSneakToggleEnabled) {
			if (isCrawling && !willStopCrawlStartSneak)
				willStopSneak = true;
			if (wantSneak && wantSprint && sneakButton.StartPressed
					&& sneakToggled) {
				willStopSneak = true;
		...
		if (isSneakToggleEnabled) {
			if (willStopCrawlStartSneak && sneakButton.StopPressed)
				willStartSneak = true;
			if (isFast && sneakButton.StopPressed
					&& !ignoreNextStopSneakButtonPressed)
				willStartSneak = true;
	...
	public void addToSendQueue() {
		...
		if (sendStatePacket) {
			SmartMovingPacketStream.sendState(SmartMovingComm.instance,
					sp.getEntityId(), state);
	...
	public boolean isStandupSprintingOrRunning() {
		return (isFast || sp.isSprinting()) && sp.onGround && !isSliding
				&& !isCrawling;
	}
	...
	public boolean doFallingAnimation() {
		if (Config.isFallAnimationEnabled())
			return !sp.onGround
					&& sp.fallDistance > Config._fallAnimationDistanceMinimum.value;
		return false;
	}
	...
	public void setLandMovementFactor(float landMovementFactor) {
		Reflect.SetField(ModifiableAttributeInstance.class,
				...
				SmartMovingInstall.ModifiableAttributeInstance_attributeValue,
				landMovementFactor);
	}
	...
	private static Class<?> ropesPlusClient = Reflect.LoadClass(
			SmartMovingInstall.class, SmartMovingInstall.RopesPlusClient,
			false);
	private static Field onZipLine = ropesPlusClient != null
			? Reflect.GetField(ropesPlusClient,
					SmartMovingInstall.RopesPlusClient_onZipLine, false)
			: null;
}