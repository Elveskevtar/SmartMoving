...
public class SmartMovingOptions extends SmartMovingClientConfig {
	public final Property<Boolean> _localUserHasChangeConfigRight = Unmodified(
			"move.global.config.right.local.user").comment(
					...
					.section();
	public final Property<Boolean> _localUserHasChangeSpeedRight = Unmodified(
			"move.global.speed.right.local.user").comment(
	...
	public final Property<Float> _perspectiveFadeFactor = PositiveFactor(
			"move.perspective.fade.factor").values(0.5F, 0.1F, 1F).comment(
					"Fading speed factor between the different perspectives (>= 0.1, <= 1, set to '1' to switch off)")
					.book("Viewpoint perspective",
							"Below you find the options to manipulate the viewpoint perspective");
	public final Property<Float> _perspectiveRunFactor = Float(
			"move.perspective.run.factor")
					.key("move.run.perspective.factor", _pre_sm_2_1)
					.defaults(1F).comment(
							"Standard sprinting perspective (set to '0' to switch off)");
	public final Property<Float> _perspectiveSprintFactor = Float(
			"move.perspective.sprint.factor")
					.key("move.sprint.perspective.factor", _pre_sm_2_1)
					.defaults(1.5F).comment(
							"Smart on ground sprinting perspective (set to '0' to switch off)");

	public final Property<Float> _angleJumpDoubleClickTicks = Positive(
			"move.jump.angle.double.click.ticks").singular().up(3F, 2F).comment(
	...
	public final Property<Boolean> _wallJumpDoubleClick = Unmodified(
			"move.jump.wall.double.click").singular().comment(
					"Whether wall jumping should be triggered by single or double clicking (and then press and holding) the jump button")
					.section();
	public final Property<Float> _wallJumpDoubleClickTicks = Positive(
			"move.jump.wall.double.click.ticks").singular().up(3F, 2F).comment(
					"The maximum number of ticks between two clicks to trigger a wall jump (>= 2, depends on \"move.jump.wall.double.click\")");

	public final Property<Boolean> _climbJumpBackHeadOnGrab = Unmodified(
			"move.jump.climb.back.head.on.grab").singular().comment(
					"Whether pressing or not pressing the grab button while climb jumping back results in a head jump")
					.section();

	public final Property<Boolean> _displayExhaustionBar = Unmodified(
			"move.gui.exhaustion.bar").singular().comment(
					"Whether to display the exhaustion bar in the game overlay")
					.section();
	public final Property<Boolean> _displayJumpChargeBar = Unmodified(
			"move.gui.jump.charge.bar").singular().comment(
	...
	public final Property<Boolean> _flyCloseToGround = Modified(
			"move.fly.ground.close")
					.comment("To switch on/off flying close to the ground")
					.section();
	public final Property<Boolean> _flyWhileOnGround = Modified(
			"move.fly.ground.collide").depends(_flyCloseToGround).comment(
					"To switch on/off flying while colliding with the grond (Relevant only if \"move.fly.ground.close\" is true)");

	public final Property<Boolean> _flyControlVertical = Unmodified(
			"move.fly.control.vertical").comment(
					"Whether flying control also depends on where the player looks vertically.")
					.section();
	public final Property<Boolean> _diveControlVertical = Unmodified(
			"move.dive.control.vertical").comment(
					"Whether diving control also depends on where the player looks vertically.");

	private final Property<Integer> _old_toggleKeyCode = Integer(
			"move.toggle.key", _pre_sm_1_7).singular().defaults(67);
	private final Property<String> _defaultConfigToggleKeyName = String(
			"move.config.toggle.default.key.name")
					.key("move.toggle.key.name", _pre_sm_3_2).singular()
					.defaults("F9")
					.source(_old_toggleKeyCode.toKeyName(), _pre_sm_1_7)
					.singular()
					.comment(
							"Key name to toggle Smart Moving features in-game (default: \"F9\")")
					.section();
	private final Property<String> _defaultGrabKeyName = String(
			"move.grab.default.key.name").singular().defaults("LCONTROL")
					.singular().comment(
							"Default key name to \"grab\" (default: \"LCONTROL\")");
	private final Property<String> _speedIncreaseKeyName = String(
			"move.speed.increase.default.key.name")
					.key("move.speed.increase.key.name", _pre_sm_3_2).singular()
					.defaults("O").singular().comment(
							"Key name to increase the moving speed ingame (default: \"O\")");
	private final Property<String> _speedDecreaseKeyName = String(
			"move.speed.decrease.default.key.name")
					.key("move.speed.decrease.key.name", _pre_sm_3_2).singular()
					.defaults("I").singular().comment(
							"Key name to decrease the moving speed ingame (default: \"I\")");

	public final Property<Integer> _defaultConfigToggleKeyCode = _defaultConfigToggleKeyName
			.toKeyCode(67);
	public final Property<Integer> _defaultGrabKeyCode = _defaultGrabKeyName
			.toKeyCode(29);
	public final Property<Integer> _defaultSpeedIncreaseKeyCode = _speedIncreaseKeyName
			.toKeyCode(24);
	public final Property<Integer> _defaultSpeedDecreaseKeyCode = _speedDecreaseKeyName
			.toKeyCode(23);

	public final Property<Boolean> _configChat = Unmodified("move.config.chat")
			.singular()
			.comment("To switch on/off option status messages via chat system")
			.book("Message Management",
	...
	public final Property<Boolean> _configChatInit = Unmodified(
			"move.config.chat.init").depends(_configChat).singular().comment(
					"To switch on/off the initial option status message when starting a game (Relevant only if \"move.config.chat\" is not false)");
	public final Property<Boolean> _configChatInitHelp = Unmodified(
			"move.config.chat.init.help").depends(_configChatInit).singular()
					.comment(
	...
	public final Property<Boolean> _configChatServer = Unmodified(
			"move.config.chat.server").depends(_configChat).singular().comment(
	...
	public final Property<Boolean> _speedChat = Unmodified("move.speed.chat")
			.singular()
			.comment("To switch on/off speed messages via chat system")
			.section();
	public final Property<Boolean> _speedChatInit = Unmodified(
			"move.speed.chat.init").depends(_speedChat).singular().comment(
					"To switch on/off the intial speed message when starting a game (Relevant only if \"move.speed.chat\" is not false)");
	public final Property<Boolean> _speedChatInitHelp = Unmodified(
			"move.speed.chat.init.help").depends(_speedChatInit).singular()
					.comment(
	...
	public final Property<Boolean> _speedChatServer = Unmodified(
			"move.config.chat.server").depends(_speedChat).singular().comment(
	...
	public static final File optionsPath = net.minecraft.client.Minecraft
	...
	public SmartMovingOptions() {
		...
		keyBindGrab = new KeyBinding("key.climb", _defaultGrabKeyCode.value,
				"key.categories.gameplay");
		keyBindConfigToggle = new KeyBinding("key.config.toggle",
				_defaultConfigToggleKeyCode.value,
				"key.categories.smartmoving");
		keyBindSpeedIncrease = new KeyBinding("key.speed.increase",
				_defaultSpeedIncreaseKeyCode.value,
				"key.categories.smartmoving");
		keyBindSpeedDecrease = new KeyBinding("key.speed.decrease",
				_defaultSpeedDecreaseKeyCode.value,
				"key.categories.smartmoving");
	}
	...
	private void writeClientConfigMessageToChat(boolean everyone) {
		String prefix = getClientEveryonePrefix("move.config.chat.client",
				everyone);
		if (SmartMovingContext.Config.enabled) {
			...
			if (name == SmartMovingProperties.Enabled
					|| (unnamed && getKeyCount() == 1))
				writeToChat(prefix + "enabled", SmartMovingInfo.ConfigChatId);
			else
				writeToChat(prefix + (unnamed ? "unnamed" : "named"),
						SmartMovingInfo.ConfigChatId, new Object[] { name });
		} else
	...
	public void writeClientSpeedMessageToChat(boolean everyone) {
		...
		String prefix = getClientEveryonePrefix("move.speed.chat.client",
				everyone);
		String key = prefix
				+ (percent.equals(SmartMovingConfig.defaultSpeedPercent)
						? "reset"
						: "change");
		writeToChat(key, SmartMovingInfo.SpeedChatId, percent);
	}

	private static String getClientEveryonePrefix(String base,
			boolean everyone) {
		String result = base + ".";
		if (everyone)
			result += "everyone.";
	...
	public void writeServerConfigMessageToChat() {
		...
		if (SmartMovingContext.Config.enabled) {
			...
			if (configName != null && !configName.isEmpty())
				writeToChat("move.config.chat.server.global.named",
						SmartMovingInfo.DefaultChatId, configName);
			else
				writeToChat("move.config.chat.server.global.unnamed",
						SmartMovingInfo.DefaultChatId);
		} else
	...
	public void writeServerReconfigMessageToChat(boolean wasEnabled,
			String username, boolean everyone) {
		if (Minecraft.getMinecraft().player.getGameProfile().getName()
				.equals(username))
			writeClientConfigMessageToChat(everyone);
		else if (_configChatServer.value) {
			if (SmartMovingContext.Config.enabled) {
				String configname = SmartMovingContext.Config._configKeyName.value;
				boolean hasConfigName = configname != null
						&& !configname.isEmpty();
				if (wasEnabled)
					if (hasConfigName)
						if (username != null)
							writeToChat(
									"move.config.chat.server.update.named.user",
									SmartMovingInfo.ConfigChatId, configname,
									username);
						else
							writeToChat("move.config.chat.server.update.named",
									SmartMovingInfo.ConfigChatId, configname);
					else if (username != null)
						writeToChat(
								"move.config.chat.server.update.unnamed.user",
								SmartMovingInfo.ConfigChatId, username);
					else
						writeToChat("move.config.chat.server.update.unnamed",
								SmartMovingInfo.ConfigChatId);
				else if (hasConfigName)
					if (username != null)
						writeToChat("move.config.chat.server.update.named.user",
								SmartMovingInfo.ConfigChatId, configname,
								username);
					else
						writeToChat("move.config.chat.server.update.named",
					...
					writeToChat("move.config.chat.server.enable.user",
							SmartMovingInfo.ConfigChatId, username);
				else
				...
				if (username != null)
					writeToChat("move.config.chat.server.disable.user",
							SmartMovingInfo.ConfigChatId, username);
				else
					writeToChat("move.config.chat.server.disable",
							SmartMovingInfo.ConfigChatId);
	...
	public void writeServerDeconfigMessageToChat() {
		if (_configChatServer.value)
			writeToChat("move.config.chat.server.local",
					SmartMovingInfo.DefaultChatId);
	}

	public void writeServerSpeedMessageToChat(String username,
			boolean everyone) {
		if (Minecraft.getMinecraft().player.getGameProfile().getName()
				.equals(username))
			...
			if (percent.equals(SmartMovingConfig.defaultSpeedPercent))
				writeToChat(prefix + "reset", SmartMovingInfo.SpeedChatId,
						username);
			else
				writeToChat(prefix + "change", SmartMovingInfo.SpeedChatId,
						percent, username);
		}
	}

	public static void writeNoRightsToChangeConfigMessageToChat(
			boolean isRemote) {
		writeToChat(
				"move.config.chat.server.illegal."
						+ (isRemote ? "remote" : "local"),
				SmartMovingInfo.ConfigChatId);
	}

	public static void writeNoRightsToChangeSpeedMessageToChat(
			boolean isRemote) {
		writeToChat(
				"move.speed.chat.server.illegal."
						+ (isRemote ? "remote" : "local"),
				SmartMovingInfo.SpeedChatId);
	}
	...
	private static void writeToChat(String key, int id, Object... parameters) {
		...
		guiChat.printChatMessageWithOptionalDeletion(
				new TextComponentString(message), id);
	}

	public static void initialize(boolean redPowerWiring,
			boolean buildCraftTransportation, boolean finiteLiquid,
			boolean betterThanWolves, boolean singlePlayerCommands,
			boolean ropesPlus, boolean aSGrapplingHook, boolean betterMisc) {
	...
	public void initializeForGameIfNeccessary() {
		PlayerControllerMP controller = Minecraft
		...
		if (isUserSpeedEnabled() && _speedChatInit.value) {
			...
			if (!speedPercent.equals(defaultSpeedPercent))
				writeToChat("move.speed.chat.client.init",
						SmartMovingInfo.DefaultChatId, speedPercent);
		}
	}
	...
	private static Field _currentGameType = Reflect.GetField(
			PlayerControllerMP.class,
			SmartMovingInstall.PlayerControllerMP_currentGameType);
}