...
public abstract class SmartMovingBase {
	...
	public Block getBlock(int x, int y, int z) {
		return SmartMovingContext.getBlock(sp.world, x, y, z);
	}

	public IBlockState getState(BlockPos blockPos) {
		return SmartMovingContext.getState(sp.world, blockPos);
	}

	public IBlockState getState(int x, int y, int z) {
		return SmartMovingContext.getState(sp.world, x, y, z);
	}

	public Material getMaterial(int x, int y, int z) {
		return SmartMovingContext.getMaterial(sp.world, x, y, z);
	}
	...
	protected void moveFlying(float moveUpward, float moveStrafing,
			float moveForward, float speedFactor, boolean treeDimensional) {
		float diffMotionXStrafing = 0, diffMotionXForward = 0,
				diffMotionZStrafing = 0, diffMotionZForward = 0;
		{
			float total = MathHelper.sqrt(
					moveStrafing * moveStrafing + moveForward * moveForward);
			if (total >= 0.01F) {
				if (total < 1.0F)
					total = 1.0F;
		...
		float rotation = treeDimensional ? sp.rotationPitch / RadiantToAngle
				: 0;
		float divingHorizontalFactor = MathHelper.cos(rotation);
		float divingVerticalFactor = -MathHelper.sin(rotation)
				* Math.signum(moveForward);

		float diffMotionX = diffMotionXForward * divingHorizontalFactor
				+ diffMotionXStrafing;
		float diffMotionY = MathHelper
				.sqrt(diffMotionXForward * diffMotionXForward
						+ diffMotionZForward * diffMotionZForward)
				* divingVerticalFactor + moveUpward;
		float diffMotionZ = diffMotionZForward * divingHorizontalFactor
				+ diffMotionZStrafing;

		float total = MathHelper.sqrt(MathHelper
				.sqrt(diffMotionX * diffMotionX + diffMotionZ * diffMotionZ)
				+ diffMotionY * diffMotionY);
		if (total > 0.01F) {
	...
	protected Block supportsCeilingClimbing(int i, int j, int k) {
		...
		if ((name.equals("tile.trapdoor") || name.equals("tile.trapdoor_iron"))
				&& !SmartMovingContext.getValue(state, BlockTrapDoor.OPEN))
			return state.getBlock();
	...
	protected float getLiquidBorder(int i, int j, int k) {
		...
		if (block == Block.getBlockFromName("water")
				|| block == Block.getBlockFromName("flowing_water"))
			return getNormalWaterBorder(i, j, k);
		if (SmartMovingOptions.hasFiniteLiquid
				&& (finiteLiquidBorder = getFiniteLiquidWaterBorder(i, j, k,
						block)) > 0)
			return finiteLiquidBorder;
		if (block == Block.getBlockFromName("lava")
				|| block == Block.getBlockFromName("flowing_lava"))
			return SmartMovingContext.Config._lavaLikeWater.value
					? getNormalWaterBorder(i, j, k)
					: 0F;

		Material material = getMaterial(i, j, k);
		if (material == null || material == Material.LAVA)
			return SmartMovingContext.Config._lavaLikeWater.value ? 1F : 0F;
	...
	protected float getNormalWaterBorder(int i, int j, int k) {
		int level = SmartMovingContext.getValue(getState(i, j, k),
				BlockLiquid.LEVEL);
		if (level >= 8)
			return 1F;
		if (level == 0)
	...
	protected float getFiniteLiquidWaterBorder(int i, int j, int k,
			Block block) {
		int type;
		if ((type = Orientation.getFiniteLiquidWater(block)) > 0) {
			if (type == 2)
				return 1F;
			if (type == 1) {
				Block aboveBlock = SmartMovingContext.getBlock(sp.world, i,
						j + 1, k);
				if (Orientation.getFiniteLiquidWater(aboveBlock) > 0)
					return 1F;
				return SmartMovingContext.getValue(getState(i, j, k),
						BlockLiquid.LEVEL) / 16F;
			}
		}
		return 0F;
	}
	...
	protected int getOnLadder(int maxResult, boolean faceOnly,
			boolean isSmall) {
	...
	protected int getOnLadderOrVine(int maxResult, boolean faceOnly,
			boolean isSmall) {
		return getOnLadderOrVine(maxResult, faceOnly, true, true, isSmall);
	}

	protected int getOnLadderOrVine(int maxResult, boolean faceOnly,
			boolean ladder, boolean vine, boolean isSmall) {
		int i = MathHelper.floor(sp.posX);
		int minj = MathHelper.floor(getBoundingBox().minY);
		int k = MathHelper.floor(sp.posZ);

		if (SmartMovingContext.Config.isStandardBaseClimb()) {
			Block block = SmartMovingContext.getBlock(sp.world, i, minj, k);
			if (ladder)
				if (vine)
					return Orientation.isClimbable(sp.world, i, minj, k) ? 1
							: 0;
				else
					return block != Block.getBlockFromName("vine")
							&& Orientation.isClimbable(sp.world, i, minj, k) ? 1
									: 0;
			else if (vine)
				return block == Block.getBlockFromName("vine")
						&& Orientation.isClimbable(sp.world, i, minj, k) ? 1
			...
			if (faceOnly)
				facedOnlyTo = Orientation.getClimbingOrientations(sp, true,
						false);

			int result = 0;
			int maxj = MathHelper.floor(sp.getEntityBoundingBox().minY
					+ Math.ceil(getBoundingBox().maxY - getBoundingBox().minY))
					- 1;
			for (int j = minj; j <= maxj; j++) {
				...
				if (ladder) {
					...
					if (localLadder) {
						localLadderOrientation = Orientation
								.getKnownLadderOrientation(sp.world, i, j, k);
						if (facedOnlyTo == null
								|| facedOnlyTo.contains(localLadderOrientation))
							result++;
					}

					for (Orientation direction : facedOnlyTo != null
							? facedOnlyTo
							: Orientation.Orthogonals) {
						if (result >= maxResult)
							return result;

						if (direction != localLadderOrientation) {
							IBlockState remoteState = getState(i + direction._i,
									j, k + direction._k);
							if (Orientation.isKnownLadder(remoteState)) {
								Orientation remoteLadderOrientation = Orientation
										.getKnownLadderOrientation(sp.world,
												i + direction._i, j,
												k + direction._k);
								if (remoteLadderOrientation
				...
				if (vine && Orientation.isVine(state))
					if (facedOnlyTo == null)
						...
						while (iterator.hasNext()) {
							...
							if (climbOrientation.hasVineOrientation(sp.world, i,
									j, k)
									&& climbOrientation.isRemoteSolid(sp.world,
											i, j, k)) {
								result++;
								break;
							}
	...
	public boolean climbingUpIsBlockedByLadder() {
		if (sp.collidedHorizontally && sp.collidedVertically && !sp.onGround
				&& esp.movementInput.moveForward > 0F) {
			Orientation orientation = Orientation.getOrientation(sp, 20F, true,
					false);
			if (orientation != null) {
				int i = MathHelper.floor(sp.posX);
				...
				if (Orientation.isLadder(getState(i, j, k)))
					return Orientation.getKnownLadderOrientation(sp.world, i, j,
							k) == orientation;
			}
		}
		return false;
		...
		if (sp.collidedHorizontally && sp.collidedVertically && !sp.onGround
			...
			if (orientation != null) {
				...
				if (Orientation.isTrapDoor(getState(i, j, k)))
					return Orientation.getOpenTrapDoorOrientation(sp.world, i,
							j, k) == orientation;
			}
		}
		return false;
	...
	private List<?> getPlayerSolidBetween(double yMin, double yMax,
			double horizontalTolerance) {
		AxisAlignedBB bb = getBoundingBox();
		bb = new AxisAlignedBB(bb.minX, yMin, bb.minZ, bb.maxX, yMax, bb.maxZ);
		return sp.world.getCollisionBoxes(sp, horizontalTolerance == 0 ? bb
				: bb.expand(horizontalTolerance, 0, horizontalTolerance));
	}
	...
	protected double getMaxPlayerSolidBetween(double yMin, double yMax,
			double horizontalTolerance) {
	...
	protected double getMinPlayerSolidBetween(double yMin, double yMax,
			double horizontalTolerance) {
		List<?> solids = getPlayerSolidBetween(yMin, yMax, horizontalTolerance);
		double result = yMax;
		for (int i = 0; i < solids.size(); i++) {
	...
	protected boolean isInLiquid() {
		return getMaxPlayerLiquidBetween(getBoundingBox().minY,
				getBoundingBox().maxY) != getBoundingBox().minY
				|| getMinPlayerLiquidBetween(getBoundingBox().minY,
						getBoundingBox().maxY) != getBoundingBox().maxY;
	}
	...
	public boolean isCollided(AxisAlignedBB box, double yMin, double yMax,
			double horizontalTolerance) {
	...
	protected boolean pushOutOfBlocks(double x, double y, double z,
		...
		if (inTranslucentBlock) {
			byte b0 = -1;
			double d5 = 9999.0D;
			if ((!isHeadspaceFree(blockpos.west(), entHeight, top))
					&& (d3 < d5)) {
				d5 = d3;
				b0 = 0;
			}
			if ((!isHeadspaceFree(blockpos.east(), entHeight, top))
					&& (1.0D - d3 < d5)) {
				d5 = 1.0D - d3;
				b0 = 1;
			}
			if ((!isHeadspaceFree(blockpos.north(), entHeight, top))
					&& (d4 < d5)) {
				d5 = d4;
				b0 = 4;
			}
			if ((!isHeadspaceFree(blockpos.south(), entHeight, top))
					&& (1.0D - d4 < d5)) {
				d5 = 1.0D - d4;
				b0 = 5;
			}
	...
	private boolean isOpenBlockSpace(BlockPos pos, boolean top) {
		...
		return !blockState.getBlock().isNormalCube(blockState, sp.world, pos)
				&& (!top || !upBlockState.getBlock().isNormalCube(blockState,
						sp.world, pos.up()));
	}
	...
	public boolean isInsideOfMaterial(Material material) {
		if (SmartMovingOptions.hasFiniteLiquid && material == Material.WATER) {
			...
			float border;
			if (l != null
					&& (border = getFiniteLiquidWaterBorder(i, j, k, l)) > 0) {
	...
	public int calculateSeparateCollisions(double x, double y, double z) {
		...
		if (flag) {
			double d9 = 0.05D;
			for (; (x != 0.0D) && (worldObj
					.getCollisionBoxes(_this,
							boundingBox.offset(x, -1.0D, 0.0D))
					.isEmpty()); d6 = x) {
				if ((x < d9) && (x >= -d9)) {
					x = 0.0D;
				} else if (x > 0.0D) {
			...
			for (; (z != 0.0D) && (worldObj
					.getCollisionBoxes(_this,
							boundingBox.offset(0.0D, -1.0D, z))
					.isEmpty()); d8 = z) {
				if ((z < d9) && (z >= -d9)) {
					z = 0.0D;
				} else if (z > 0.0D) {
		...
		List list1 = worldObj.getCollisionBoxes(_this,
				boundingBox.expand(x, y, z));
		AxisAlignedBB axisalignedbb = boundingBox;
		AxisAlignedBB axisalignedbb1;
		for (Iterator iterator = list1.iterator(); iterator
				.hasNext(); y = axisalignedbb1.calculateYOffset(boundingBox,
						y)) {
		...
		for (Iterator iterator8 = list1.iterator(); iterator8
				.hasNext(); x = axisalignedbb2.calculateXOffset(boundingBox,
						x)) {
			axisalignedbb2 = (AxisAlignedBB) iterator8.next();
		}
		boundingBox = boundingBox.offset(x, 0.0D, 0.0D);
		for (Iterator iterator8 = list1.iterator(); iterator8
				.hasNext(); z = axisalignedbb2.calculateZOffset(boundingBox,
						z)) {
			axisalignedbb2 = (AxisAlignedBB) iterator8.next();
		}
		...
		if ((stepHeight > 0.0F) && (flag1) && ((d6 != x) || (d8 != z))) {
			...
			y = stepHeight;
			List list = worldObj.getCollisionBoxes(_this,
					boundingBox.expand(d6, y, d8));
			...
			for (Iterator iterator1 = list.iterator(); iterator1
					.hasNext(); d12 = axisalignedbb6
							.calculateYOffset(axisalignedbb5, d12)) {
			...
			for (Iterator iterator2 = list.iterator(); iterator2
					.hasNext(); d18 = axisalignedbb7
							.calculateXOffset(axisalignedbb4, d18)) {
			...
			for (Iterator iterator3 = list.iterator(); iterator3
					.hasNext(); d19 = axisalignedbb8
							.calculateZOffset(axisalignedbb4, d19)) {
			...
			for (Iterator iterator4 = list.iterator(); iterator4
					.hasNext(); d20 = axisalignedbb9
							.calculateYOffset(axisalignedbb13, d20)) {
			...
			for (Iterator iterator5 = list.iterator(); iterator5
					.hasNext(); d21 = axisalignedbb10
							.calculateXOffset(axisalignedbb13, d21)) {
			...
			for (Iterator iterator6 = list.iterator(); iterator6
					.hasNext(); d22 = axisalignedbb11
							.calculateZOffset(axisalignedbb13, d22)) {
			...
			for (Iterator iterator7 = list.iterator(); iterator7
					.hasNext(); y = axisalignedbb12
							.calculateYOffset(boundingBox, y)) {
	...
	public void correctOnUpdate(boolean isSmall,
			boolean reverseMaterialAcceleration) {
		...
		if (f < 0.05F && f > 0.02 && isSmall) {
			...
			for (; sp.renderYawOffset
					- sp.prevRenderYawOffset < -180F; sp.prevRenderYawOffset -= 360F) {
			}
			for (; sp.renderYawOffset
					- sp.prevRenderYawOffset >= 180F; sp.prevRenderYawOffset += 360F) {
			}
	...
	protected double getGapUnderneight() {
		return getBoundingBox().minY - getMaxPlayerSolidBetween(
				getBoundingBox().minY - 1.1D, getBoundingBox().minY, 0);
	}

	protected double getGapOverneight() {
		return getMinPlayerSolidBetween(getBoundingBox().maxY,
				getBoundingBox().maxY + 1.1D, 0) - getBoundingBox().maxY;
	}

	public double getOverGroundHeight(double maximum) {
		if (esp != null)
			return (getBoundingBox().minY - getMaxPlayerSolidBetween(
					getBoundingBox().minY - maximum, getBoundingBox().minY, 0));
		return (getBoundingBox().minY + 1D
				- getMaxPlayerSolidBetween(getBoundingBox().minY - maximum + 1D,
						getBoundingBox().minY + 1D, 0.1));
	}
	...
	public void reverseHandleMaterialAcceleration() {
		...
		AxisAlignedBB axisalignedbb = getBoundingBox()
				.expand(0.0D, -0.40000000596046448D, 0.0D)
		...
		if (!_this.isAreaLoaded(new BlockPos(i, k, i1), new BlockPos(j, l, j1),
				true))
			return;
					...
					if (iblockstate.getMaterial() == material) {
						double d0 = l1 + 1
								- BlockLiquid.getLiquidHeightPercent(
										iblockstate.getValue(BlockLiquid.LEVEL)
												.intValue());
						if (l >= d0) {
							vec3 = block.modifyAcceleration(_this, blockpos,
									entity, vec3);
						}
					}
				}
