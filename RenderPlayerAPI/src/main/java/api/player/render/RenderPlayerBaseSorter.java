...
{
	public RenderPlayerBaseSorter(List<String> list, Map<String, String[]> allBaseSuperiors, Map<String, String[]> allBaseInferiors, String methodName)
	{
		this.list = list;
			...
			for(int i = 0; i < list.size() - 1; i++)
				for(int n = i + 1; n < list.size(); n++)
				{
					...
					if(leftWantsToBeInferiorToRight && rightWantsToBeInferiorToLeft)
						throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + methodName + "'. '" + left + "' wants to be inferior to '" + right + "' and '" + right + "' wants to be inferior to '" + left + "'");
					if(leftWantsToBeSuperiorToRight && rightWantsToBeSuperiorToLeft)
						throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + methodName + "'. '" + left + "' wants to be superior to '" + right + "' and '" + right + "' wants to be superior to '" + left + "'");

					if(leftWantsToBeInferiorToRight && leftWantsToBeSuperiorToRight)
						throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + methodName + "'. '" + left + "' wants to be superior and inferior to '" + right + "'");
					if(rightWantsToBeInferiorToLeft && rightWantsToBeSuperiorToLeft)
						throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + methodName + "'. '" + right + "' wants to be superior and inferior to '" + left + "'");
				}
	...
	private Set<String> build(String type, Set<String> inferiors, String startType)
	{
		...
		while(iter.hasNext())
		{
			...
			if(inferiorType == startType)
				throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + methodName + "'. Circular superiosity found including '" + startType + "'");
			...
			{
				throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + methodName + "'. Circular superiosity found including '" + inferiorType + "'", uoe);
			}
