...
public class SMOptions extends SMClientConfig {
	...
	private static Field _currentGameType = Reflect.GetField(PlayerControllerMP.class,
			SMInstall.PlayerControllerMP_currentGameType);
	...
	public SMOptions() {
		...
		keyBindConfigToggle = new KeyBinding("key.config.toggle", _defaultConfigToggleKeyCode.value,
				"key.categories.smartmoving");
		keyBindSpeedIncrease = new KeyBinding("key.speed.increase", _defaultSpeedIncreaseKeyCode.value,
				"key.categories.smartmoving");
		keyBindSpeedDecrease = new KeyBinding("key.speed.decrease", _defaultSpeedDecreaseKeyCode.value,
				"key.categories.smartmoving");
	}
	...
	private void writeClientConfigMessageToChat(boolean everyone) {
		...
		if (SMContext.Config.enabled) {
			String name = SMContext.Config._configKeyName.value;
			if (name.isEmpty())
				name = null;
			...
			if (name == SMProperties.Enabled || (unnamed && getKeyCount() == 1))
				writeToChat(prefix + "enabled", ConfigChatId);
			else
				writeToChat(prefix + (unnamed ? "unnamed" : "named"), ConfigChatId, new Object[] { name });
		} else
			writeToChat(prefix + "disabled", ConfigChatId);
	}
	...
	public void writeClientSpeedMessageToChat(boolean everyone) {
		...
		writeToChat(key, SpeedChatId, percent);
	}
	...
	public void writeServerConfigMessageToChat() {
		...
		if (SMContext.Config.enabled) {
			String configName = SMContext.Config._configKeyName.value;
			if (configName != null && !configName.isEmpty())
				writeToChat("move.config.chat.server.global.named", DefaultChatId, configName);
			else
				writeToChat("move.config.chat.server.global.unnamed", DefaultChatId);
		} else
			writeToChat("move.config.chat.server.disable", DefaultChatId);
	}
		...
		if (Minecraft.getMinecraft().player.getGameProfile().getName().equals(username))
			...
			if (SMContext.Config.enabled) {
				...
				if (wasEnabled)
					if (hasConfigName)
						if (username != null)
							writeToChat("move.config.chat.server.update.named.user", ConfigChatId, configname,
									username);
						else
							writeToChat("move.config.chat.server.update.named", ConfigChatId, configname);
					else if (username != null)
						writeToChat("move.config.chat.server.update.unnamed.user", ConfigChatId, username);
					else
						writeToChat("move.config.chat.server.update.unnamed", ConfigChatId);
				else if (hasConfigName)
					if (username != null)
						writeToChat("move.config.chat.server.update.named.user", ConfigChatId, configname, username);
					else
						writeToChat("move.config.chat.server.update.named", ConfigChatId, configname);
				else if (username != null)
					writeToChat("move.config.chat.server.enable.user", ConfigChatId, username);
				else
					writeToChat("move.config.chat.server.enable", ConfigChatId);
			} else if (wasEnabled)
				if (username != null)
					writeToChat("move.config.chat.server.disable.user", ConfigChatId, username);
				else
					writeToChat("move.config.chat.server.disable", ConfigChatId);
		}
	}

	public void writeServerDeconfigMessageToChat() {
		if (_configChatServer.value)
			writeToChat("move.config.chat.server.local", DefaultChatId);
	}
	...
	public void writeServerSpeedMessageToChat(String username, boolean everyone) {
		if (Minecraft.getMinecraft().player.getGameProfile().getName().equals(username))
			...
			if (percent.equals(SMConfig.defaultSpeedPercent))
				writeToChat(prefix + "reset", SpeedChatId, username);
			else
				writeToChat(prefix + "change", SpeedChatId, percent, username);
		}
	}

	public static void writeNoRightsToChangeConfigMessageToChat(boolean isRemote) {
		writeToChat("move.config.chat.server.illegal." + (isRemote ? "remote" : "local"), ConfigChatId);
	}

	public static void writeNoRightsToChangeSpeedMessageToChat(boolean isRemote) {
		writeToChat("move.speed.chat.server.illegal." + (isRemote ? "remote" : "local"), SpeedChatId);
	}

	private static void writeToChat(String key, int id, Object... parameters) {
		String message = parameters == null || parameters.length == 0 ? I18n.format(key) : I18n.format(key, parameters);
	...
	public void initializeForGameIfNeccessary() {
		...
		if (isUserSpeedEnabled() && _speedChatInit.value) {
			...
			if (!speedPercent.equals(defaultSpeedPercent))
				writeToChat("move.speed.chat.client.init", DefaultChatId, speedPercent);
		}
	}
}
