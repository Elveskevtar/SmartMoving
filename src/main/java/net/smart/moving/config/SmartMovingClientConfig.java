...
public class SmartMovingClientConfig extends SmartMovingConfig {
	...
	public boolean isJumpExhaustionEnabled(int speed, int type) {
		...
		if (type == ClimbUp || type == ClimbUpHandsOnly || type == ClimbBackUp
				|| type == ClimbBackUpHandsOnly || type == ClimbBackHead
				|| type == ClimbBackHeadHandsOnly)
	...
	public float getJumpExhaustionGain(int speed, int type, float jumpCharge) {
		...
		float result = _baseExhautionGainFactor.value
		...
		if (type == ChargeUp) {
			...
			result += _baseExhautionGainFactor.value
					* _jumpExhaustionGainFactor.value
					* _upJumpExhaustionGainFactor.value
					* _jumpChargeExhaustionGainFactor.value
					* Math.min(jumpCharge, _jumpChargeMaximum.value)
					/ _jumpChargeMaximum.value;
		}
	...
	public float getJumpExhaustionStop(int speed, int type, float jumpCharge) {
		...
		if (type == ChargeUp) {
			...
			result -= _jumpExhaustionStopFactor.value
	...
	public float getJumpChargeFactor(float jumpCharge) {
		...
		return 1F + jumpCharge / _jumpChargeMaximum.value
				* (_jumpChargeFactor.value - 1F);
	}
	...
	public float getJumpHorizontalFactor(int speed, int type) {
		...
		else if (speed == Standing && type != ClimbBackUp
				&& type != ClimbBackUpHandsOnly && type != ClimbBackHead
				&& type != ClimbBackHeadHandsOnly)
			result *= 0F;
			...
			for (int i = Sprinting; i <= Standing; i++)
				for (int n = Up; n <= WallHeadSlide; n++)
					if (isJumpExhaustionEnabled(i, n))
						for (int t = 0; t <= 1; t++)
							result = max(result, getJumpExhaustionStop(i, n, t)
									+ getJumpExhaustionGain(i, n, t));
	...
	private static float max(float value, float valueOrInfinite) {
		return valueOrInfinite == java.lang.Float.POSITIVE_INFINITY ? value
				: Math.max(value, valueOrInfinite);
	}

	public float getFactor(boolean hunger, boolean onGround, boolean isStanding,
			boolean isStill, boolean isSneaking, boolean isRunning,
			boolean isSprinting, boolean isClimbing, boolean isClimbCrawling,
			boolean isCeilingClimbing, boolean isDipping, boolean isSwimming,
			boolean isDiving, boolean isCrawling, boolean isCrawlClimbing) {
		isClimbing |= isClimbCrawling;
		isCrawling |= isCrawlClimbing;
		boolean actionOverGound = isClimbing || isCeilingClimbing || isDiving
				|| isSwimming;
		boolean airBorne = !onGround && !actionOverGound;
		isStanding = actionOverGound ? isStill : isStanding;
		isSneaking = isSneaking & !isStanding;

		float factor = hunger ? _baseHungerGainFactor.value
				: _baseExhautionLossFactor.value;
		if (airBorne)
			factor *= hunger ? 0F : _fallExhautionLossFactor.value;
		else if (isSprinting)
			factor *= hunger ? _sprintingHungerGainFactor.value
					: _sprintingExhautionLossFactor.value;
		else if (isRunning)
			factor *= hunger ? _runningHungerGainFactor.value
					: _runningExhautionLossFactor.value;
		else if (isSneaking)
			factor *= hunger ? _sneakingHungerGainFactor.value
					: _sneakingExhautionLossFactor.value;
		else if (isStanding)
			factor *= hunger ? _standingHungerGainFactor.value
					: _standingExhautionLossFactor.value;
		else
			factor *= hunger ? _walkingHungerGainFactor.value
					: _walkingExhautionLossFactor.value;

		if (isClimbing)
			factor *= hunger ? _climbingHungerGainFactor.value
					: _climbingExhaustionLossFactor.value;
		else if (isCrawling)
			factor *= hunger ? _crawlingHungerGainFactor.value
					: _crawlingExhaustionLossFactor.value;
		else if (isCeilingClimbing)
			factor *= hunger ? _ceilClimbingHungerGainFactor.value
					: _ceilClimbingExhaustionLossFactor.value;
		else if (isSwimming)
			factor *= hunger ? _swimmingHungerGainFactor.value
					: _swimmingExhaustionLossFactor.value;
		else if (isDiving)
			factor *= hunger ? _divingHungerGainFactor.value
					: _divingExhaustionLossFactor.value;
		else if (isDipping)
			factor *= hunger ? _dippingHungerGainFactor.value
					: _dippingExhaustionLossFactor.value;
		else if (onGround)
			factor *= hunger ? _normalHungerGainFactor.value
					: _normalExhaustionLossFactor.value;
		else
			factor *= hunger ? _normalHungerGainFactor.value
					: _normalExhaustionLossFactor.value;
